...
public static class ConfigureViewModels
{
    public static IServiceCollection AddAppViewModels(this IServiceCollection services)
    {
        ...
        services.AddSingleton<IConsumerApi>(serviceProvider =>
        {
            ...
            var storeApi = new ConsumerApi.ConsumerApi(lzHttpClient);
            return storeApi;
        });

        // Register the modules used from the Client SDK.
        services.AddSingleton<IPublicModuleClient>(provider => provider.GetRequiredService<IConsumerApi>());
        services.AddSingleton<IConsumerModuleClient>(provider => provider.GetRequiredService<IConsumerApi>());
